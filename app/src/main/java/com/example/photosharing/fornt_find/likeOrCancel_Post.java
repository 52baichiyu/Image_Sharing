...
public class likeOrCancel_Post {
    private Gson gson = new Gson();


     void post(String shareId,String userId){
...
// url路径
    ...
    private final Callback callback1 = new Callback() {
        ...
        public void onResponse(@NonNull Call call, Response response) throws IOException {
            //TODO 请求成功处理
            Type jsonType = new TypeToken<ResponseBody1<Object>>() {
            }.getType();
            System.out.println("点赞取消接口成功返回！");
            // 获取响应体的json串
            String body = response.body().string();
            Log.d("info", body);
            // 解析json串到自己封装的状态
